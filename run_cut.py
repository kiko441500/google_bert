...
class CutProcessor(DataProcessor):
    ...
    def _create_example(self, lines, set_type):#读取样本，并返回样本和标签
        examples = []
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("loss"):
        ...
        logits = tf.matmul(output_layer, output_weight, transpose_b=True)#词数量*768 和 768*7的矩阵相乘，得到句子中每个词对应的logits
        logits = tf.nn.bias_add(logits, output_bias)#bias初始化时候为0,
        logits = tf.reshape(logits, [-1, FLAGS.max_seq_length, num_labels])#将logits重新转换为以句子为单位，[batchsize，max_seq_length，num_labels]
        log_probs = tf.nn.log_softmax(logits, axis=-1)#对所有
